...
def plot_ternary_system(tern_sys, binary_data, param_format='combined', temp_slider=[0, 300], t_incr=5, delta=0.025):
    ...
    print(binary_L_dict)
    fitorpred = {}
    for bin_sys in binary_L_dict.keys():
        fitorpred[bin_sys] = 'fit'  # or 'pred' based on your logic

    plotter = ternary_gtx_plotter(tern_sys, data_dir, interp_type="linear", param_format=param_format,
                                  L_dict=binary_L_dict, temp_slider=temp_slider, T_incr=t_incr, delta=delta, fit_or_pred=fitorpred)
    plotter.interpolate()
    ...
    ploff.plot(tern_fig, filename=f'./figures/{"".join(tern_sys)}_system_highres.html', auto_open=True)
...
if __name__ == "__main__":
    ...
    plot_ternary_system('Ga-Fe-Ni', fitted_data, t_incr=0.5, delta=0.01)
