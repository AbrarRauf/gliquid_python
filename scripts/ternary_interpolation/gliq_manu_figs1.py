...
def eutectic_fig():
    ...
    df_eut["Eutectic Composition"] = df_eut["Eutectic Composition"].apply(
        lambda x: ast.literal_eval(x)
    )
    df_eut["Exp Eut Composition"] = df_eut["Exp Eut Composition"].apply(
        lambda x: ast.literal_eval(x)
    )
    ...
    print(
        f"Eutectic temperature std. dev. of differences (post-NaN filter): {temp_std:.2f} K"
    )
    print(
        f"Average Cartesian composition distance (post-NaN filter): {mean_composition_distance:.4f}"
    )
    print(
        f"Std. dev. of Cartesian composition distance (post-NaN filter): {composition_std:.4f}"
    )
    ...
    for _, row in df.iterrows():
        ...
        eutectic_traces.append(
            go.Scatterternary(
                a=[eut[0]],
                b=[eut[1]],
                c=[eut[2]],
                mode="markers",
                name=f"{sys} (Simulated)",
                marker=dict(
                    color=color,
                    symbol="circle",
                    size=marker_size,
        ...
        if len(tern_points) < min_valid_points:
            ...
            tern_points = np.array(
                [
                    to_ternary_coords(p_a),
                    to_ternary_coords(p_b),
                    to_ternary_coords(p_c),
                ],
                dtype=float,
            )
            tern_points = np.clip(tern_points, 0.0, 1.0)
    ...
    if DRAW_ELLIPSES_ON_TOP:
        ...
        fig = go.Figure(
            [triangle_trace] + exp_traces + eutectic_traces + ellipse_traces
        )
    else:
        fig = go.Figure(
            [triangle_trace] + ellipse_traces + exp_traces + eutectic_traces
        )
...
def inter_figure_raw():
    ...
    plt.scatter(
        df["melting_point_k"], df["gliq_melting_temp"], c="blue", s=20, alpha=0.7
    )
...
def inter_figure(interactive_html=False):
    # Font settings - change these to customize appearance
    FIGURE_SIZE = (8, 6)
    AXIS_LIMITS = (550, 2600)
    FIGURE_PADDING = 0.6
    MARKER_SIZE = 95
    TICK_LABEL_SIZE = 19
    AXIS_LABEL_SIZE = 20
    ...
    meta_data_path = (
        "all_dumps/gliq_manu_forreal_plusML/ternary_Gliq_meta_final_linear.json"
    )
    ...
    for label, subset in [
        ("All fitted", df[~df["contains_pred"]]),
        ...
        subset_mape = (
            np.mean(np.abs(subset_differences / subset["melting_point_k"])) * 100
        )
    ...
    def add_compound_labels(ax, plot_df):
        ...
        label_df = plot_df.dropna(
            subset=["melting_point_k", "gliq_melting_temp", "reduced_formula"]
        ).copy()
        xlim = ax.get_xlim()
        ylim = ax.get_ylim()
        label_df = label_df[
        ...
        label_df["abs_error"] = (
            label_df["gliq_melting_temp"] - label_df["melting_point_k"]
        ).abs()
        ...
        norm_points_by_index = {
            idx: np.array([row["_nx"], row["_ny"]]) for idx, row in label_df.iterrows()
        }

        near_fit_candidate_count = min(
            max(LABEL_NEAR_FIT_MIN_COUNT * 3, LABEL_MAX_COUNT // 3), len(label_df)
        )
        near_fit_df = label_df.sort_values("_near_fit", ascending=True).head(
            near_fit_candidate_count
        )
        high_error_df = label_df.sort_values("abs_error", ascending=False).head(
            LABEL_MAX_COUNT
        )
        ...
        for row in selected:
            if (
                accepted_count >= LABEL_MAX_COUNT
                and accepted_near_fit_count >= LABEL_NEAR_FIT_MIN_COUNT
            ):
                break
            if (
                accepted_count >= LABEL_MAX_COUNT
                and row.name not in near_fit_candidate_indices
            ):
                continue
            ...
            other_marker_boxes = [
                box for idx, box in marker_box_by_index.items() if idx != row.name
            ]
            point = norm_points_by_index[row.name]
            other_norm_points = [
                point for idx, point in norm_points_by_index.items() if idx != row.name
            ]
            nearest_neighbor = min(
                (np.linalg.norm(point - other) for other in other_norm_points),
                default=np.inf,
            )
            allow_direct_label = all(
                np.linalg.norm(point - other) >= LABEL_MIN_SPACING
                for other in direct_label_points
            )
            prefer_connector = (
                row["_near_fit"] < LABEL_CONNECTOR_NEIGHBOR_RADIUS
                and nearest_neighbor < LABEL_CONNECTOR_NEIGHBOR_RADIUS
            )
            point_offsets = (
            ...
            point_offsets = [(*offset, False) for offset in point_offsets] + [
                (*offset, True) for offset in connector_offsets
            ]
            ...
            for dx_pt, dy_pt, use_connector, allow_reference_crossing in point_offsets:
                ...
                segment = (
                    (tuple(point_disp), tuple(text_disp)) if use_connector else None
                )
                clean = not any(
                    boxes_overlap(text_box, box)
                    for box in accepted_boxes + other_marker_boxes
                )
                clean = clean and not boxes_overlap(text_box, own_marker_box, pad=-2)
                clean = clean and not any(
                    not (axes_box[0] <= text_box[i] <= axes_box[2]) for i in [0, 2]
                )
                clean = clean and not any(
                    not (axes_box[1] <= text_box[i] <= axes_box[3]) for i in [1, 3]
                )
                clean = clean and not any(
                    segment_intersects_box(other, text_box)
                    for other in accepted_segments + reference_segments
                )

                if segment is not None:
                    blocked_segments = (
                        accepted_segments
                        if allow_reference_crossing
                        else accepted_segments + reference_segments
                    )
                    clean = clean and not any(
                        segments_cross(segment[0], segment[1], other[0], other[1])
                        for other in blocked_segments
                    )
                    clean = clean and not any(
                        segment_intersects_box(segment, box)
                        for box in accepted_boxes + other_marker_boxes
                    )
                ...
                if clean:
                    ...
                    if segment is not None:
                        ...
                        ax.annotate(
                            ...
                            xytext=text_data,
                            arrowprops=dict(
                                arrowstyle="-", color=LABEL_CONNECTOR_COLOR, lw=0.8
                            ),
                            zorder=4,
                        )
                    break
    ...
    if COLOR_BY_FIT_TYPE:
        ...
        df["color"] = df["type"].map(
            {"congruent": "tab:cyan", "non-congruent": "tab:orange"}
        )
    else:
    ...
    for fit_type in [False, True]:
        ...
        if len(subset) > 0:
            plt.scatter(
                ...
                linewidths=0,
                label=(
                    "Contains predicted binaries" if fit_type else "All fitted binaries"
                )
                if COLOR_BY_FIT_TYPE
                else None,
            )
    ...
    plt.xlabel(
        "MPDS Congruent Melting Temperature (K)",
        fontsize=AXIS_LABEL_SIZE,
        fontweight="bold",
    )
    plt.ylabel(
        "Predicted Melting Temperature (K)", fontsize=AXIS_LABEL_SIZE, fontweight="bold"
    )
    ...
    label_artists = (ax.xaxis.label, ax.yaxis.label)
    plt.savefig(
        "figures/inter_figure.png",
        dpi=300,
        bbox_inches="tight",
        bbox_extra_artists=label_artists,
    )
    plt.savefig(
        "figures/inter_figure.svg",
        dpi=300,
        bbox_inches="tight",
        bbox_extra_artists=label_artists,
    )
    ...
    if interactive_html:
        plot_df = df.rename(
            columns={
                "reduced_formula": "Compound",
                "system_key": "System",
                "type": "Congruency",
                "melting_point_k": "MPDS temperature (K)",
                "gliq_melting_temp": "Predicted temperature (K)",
            }
        ).copy()
        plot_df["Error (K)"] = (
            plot_df["Predicted temperature (K)"] - plot_df["MPDS temperature (K)"]
        )
        plot_df["Fit type"] = np.where(
            plot_df["contains_pred"],
            "Contains predicted binaries",
            "All fitted binaries",
        )
        if COLOR_BY_FIT_TYPE:
            color_column = "Fit type"
            color_map = {
                "All fitted binaries": "#1f77b4",
                "Contains predicted binaries": "#ff7f0e",
            }
        elif COLOR_BY_CONGRUENCY:
            color_column = "Congruency"
            color_map = {"congruent": "#17becf", "non-congruent": "#ff7f0e"}
        else:
            color_column = "Marker group"
            plot_df[color_column] = "Compounds"
            color_map = {"Compounds": "#1f77b4"}

        interactive_fig = px.scatter(
            plot_df,
            x="MPDS temperature (K)",
            y="Predicted temperature (K)",
            color=color_column,
        ...
        interactive_fig.update_traces(marker=dict(size=13))
        interactive_fig.add_shape(
            type="line",
            x0=min_val,
            y0=min_val,
            x1=max_val,
        ...
        interactive_fig.update_xaxes(
            range=list(AXIS_LIMITS), mirror=True, showline=True, linecolor="black"
        )
        interactive_fig.update_yaxes(
            range=list(AXIS_LIMITS),
            mirror=True,
            showline=True,
            linecolor="black",
            scaleanchor="x",
        ...
        interactive_fig.write_html(
            "figures/inter_figure_interactive.html", include_plotlyjs=True
        )
...
def inter_figure_filtered():
    ...
    meta_data_path = (
        "all_dumps/gliq_manu_test7_linear/ternary_Gliq_meta_final_linear.json"
    )
    ...
    df_filtered.to_excel(
        "all_dumps/gliq_manu_test7_linear/ternary_Gliq_mps_final_linear_filtered.xlsx",
        index=False,
    )
    ...
    differences_filtered = (
        df_filtered["gliq_melting_temp"] - df_filtered["melting_point_k"]
    )
    ...
    print(
        f"Average RMSE between interpolated and MPDS values (filtered): {rmse_filtered:.2f} K"
    )
    print(
        f"Average MAE between interpolated and MPDS values (filtered): {mae_filtered:.2f} K"
    )
    print(f"Standard deviation of differences (filtered): {std_dev_filtered:.2f} K")
    print(
        f"Standard deviation of differences (filtered, formula): {std_dev_filtered_formula:.2f} K"
    )
    ...
    # Plot the reference y=x line
    min_val = min(
        df_filtered["melting_point_k"].min(), df_filtered["gliq_melting_temp"].min()
    )
    max_val = max(
        df_filtered["melting_point_k"].max(), df_filtered["gliq_melting_temp"].max()
    )
    plt.plot([min_val, max_val], [min_val, max_val], "k--", label="y = x")
...
def inter_figure_correction():
    ...
    CORRECTION_FAILURES = {"Er(CuGe)2", "Tm(CuGe)2", "CeZnIn"}
    CORRECTION_FAILURE_COLOR = "#2CA02C"
    ...
    def format_and_save(fig, ax, cbar, filename):
        ...
        label_artists = (ax.xaxis.label, ax.yaxis.label, cbar.ax.yaxis.label)
        fig.savefig(
    ...
    correction_failure_mask = df["reduced_formula"].isin(CORRECTION_FAILURES)

    def plot_corrected_predictions(ax):
        scatter = ax.scatter(
            df.loc[~correction_failure_mask, "melting_point_k"],
            df.loc[~correction_failure_mask, "final_gliq_temp"],
            c=l0_tern_kj[~correction_failure_mask],
            s=MARKER_SIZE,
            cmap="RdBu_r",
            edgecolors="black",
            linewidths=1,
            ...
            zorder=3,
            vmin=-abs(l0_tern_kj).max(),
            vmax=abs(l0_tern_kj).max(),
        )
        ax.scatter(
            df.loc[correction_failure_mask, "melting_point_k"],
            df.loc[correction_failure_mask, "final_gliq_temp"],
            c=CORRECTION_FAILURE_COLOR,
            s=MARKER_SIZE,
            edgecolors="black",
            linewidths=1,
            label="Correction failure",
            zorder=4,
        )
        return scatter

    metric_df = df.dropna(
        subset=["melting_point_k", "initial_gliq_temp", "final_gliq_temp"]
    )
    reference_temps = metric_df["melting_point_k"]

    def error_metrics(column):
        errors = metric_df[column] - reference_temps
        return {
            "RMSE": np.sqrt(np.mean(errors**2)),
            "MAE": np.mean(np.abs(errors)),
            "MAPE": np.mean(np.abs(errors / reference_temps)) * 100,
            "Error standard deviation": np.std(errors),
        }

    initial_metrics = error_metrics("initial_gliq_temp")
    corrected_metrics = error_metrics("final_gliq_temp")
    print(f"Correction error metrics ({len(metric_df)} compounds):")
    for metric, unit in [
        ("RMSE", "K"),
        ("MAE", "K"),
        ("MAPE", "%"),
        ...
        reduction = (
            100
            * (initial_metrics[metric] - corrected_metrics[metric])
            / initial_metrics[metric]
        )
        print(
            f"  {metric}: {initial_metrics[metric]:.2f} -> {corrected_metrics[metric]:.2f} {unit} "
    ...
    # Plot corrected temperatures, with known failures outside the l0_tern colormap
    scatter = plot_corrected_predictions(ax)

    # Add colorbar
    cbar = plt.colorbar(scatter, ax=ax)
    cbar.set_label(
        "Ternary L0-term (kJ/mol)", rotation=90, labelpad=20, fontweight="bold"
    )

    plt.plot(AXIS_LIMITS, AXIS_LIMITS, "k--", zorder=0)
    ...
    plt.ylabel(
        "Interpolated Melting Temperature (K)",
        fontsize=AXIS_LABEL_SIZE,
        fontweight="bold",
    )
...
def inter_figure_three_panel():
    ...
    inter_df = pd.read_excel(
        "all_dumps/gliq_manu_forreal_plusML/ternary_Gliq_mps_final_linear.xlsx"
    )
    inter_df["elements"] = inter_df["elements"].apply(ast.literal_eval)
    inter_df["system_key"] = inter_df["elements"].apply(
        lambda elements: "-".join(sorted(elements))
    )
    with open(
        "all_dumps/gliq_manu_forreal_plusML/ternary_Gliq_meta_final_linear.json", "r"
    ) as file:
        meta_data = json.load(file)
    inter_df["contains_pred"] = inter_df.apply(
        lambda row: (
            meta_data.get(
                f"{row['system_key']}__idx_{row['source_row_idx']}__phase_{row['reduced_formula']}",
                meta_data.get(row["system_key"], {}),
    ...
    l0_tern_kj = correction_df["l0_tern"] / 1000
    color_limit = float(abs(l0_tern_kj).max())
    ...
    fig.get_layout_engine().set(w_pad=0, h_pad=0, wspace=PANEL_SPACING, hspace=0)

    colors = {False: "tab:blue", True: "tab:orange"}
    for fit_type in [False, True]:
        subset = inter_df[inter_df["contains_pred"] == fit_type]
        axes[0].scatter(
            subset["melting_point_k"],
            subset["gliq_melting_temp"],
            c=colors[fit_type],
            ...
            edgecolors="none",
            linewidths=0,
            label="Contains predicted binaries" if fit_type else "All fitted binaries",
        )

    axes[1].scatter(
        correction_df["melting_point_k"],
        correction_df["initial_gliq_temp"],
        c="gray",
        s=MARKER_SIZE,
        label="Initial prediction",
        zorder=2,
    ...
    corrected_scatter = axes[2].scatter(
        correction_df["melting_point_k"],
        correction_df["final_gliq_temp"],
        **scatter_kwargs,
    )
    ...
    for index, ax in enumerate(axes):
        ax.plot(AXIS_LIMITS, AXIS_LIMITS, "k--", zorder=0)
        ax.set_xlim(*AXIS_LIMITS)
        ax.set_ylim(*AXIS_LIMITS)
        ax.set_aspect("equal", adjustable="box")
        ax.tick_params(
    ...
    cbar = fig.colorbar(
        corrected_scatter, ax=axes, pad=0.015, fraction=0.025, aspect=35
    )
    cbar.set_label(
        "Ternary L0-term (kJ/mol)",
        rotation=90,
        labelpad=20,
        fontsize=AXIS_LABEL_SIZE,
        fontweight="bold",
    ...
    label_artists = (x_label, y_label, cbar.ax.yaxis.label)
    fig.savefig(
        "figures/inter_figure_three_panel.png",
        dpi=300,
        bbox_inches="tight",
        bbox_extra_artists=label_artists,
    )
    fig.savefig(
        "figures/inter_figure_three_panel.svg",
        dpi=300,
        bbox_inches="tight",
        bbox_extra_artists=label_artists,
    )
...
def inter_figure_error_metrics():
    ...
    print(
        f"MAE range: {df_filtered['mae_avg'].min():.2f} - {df_filtered['mae_avg'].max():.2f}"
    )
    print(
        f"RMSE range: {df_filtered['rmse_avg'].min():.2f} - {df_filtered['rmse_avg'].max():.2f}"
    )
...
def ternary_hull_metrics(include_predicted=True):
    ...
    df_filtered["n_ternary_compounds"] = df_filtered["system_key"].apply(
        lambda x: (
            meta_data.get(x, {}).get("ternary_meta", {}).get("n_ternary_compounds", 0)
        )
    )
    df_filtered["deepest_formation_energy"] = df_filtered["system_key"].apply(
        lambda x: (
            meta_data.get(x, {})
            .get("ternary_meta", {})
            .get("deepest_formation_energy", 0)
        )
    )

    # Convert deepest formation energy to absolute values for better visualization
    df_filtered["abs_deepest_formation_energy"] = np.abs(
        df_filtered["deepest_formation_energy"]
    )
    ...
    # Print statistics
    print(
        f"Number of ternary compounds range: {df_filtered['n_ternary_compounds'].min():.0f} - {df_filtered['n_ternary_compounds'].max():.0f}"
    )
    print(
        f"Deepest formation energy range: {df_filtered['deepest_formation_energy'].min():.0f} - {df_filtered['deepest_formation_energy'].max():.0f}"
    )
    print(
        f"|Deepest formation energy| range: {df_filtered['abs_deepest_formation_energy'].min():.0f} - {df_filtered['abs_deepest_formation_energy'].max():.0f}"
    )
...
def binary_L_parameter_analysis(include_predicted=True, deviation_metric="mad"):
    ...
    df_filtered[f"L0_a_{deviation_metric}"] = df_filtered["system_key"].apply(
        lambda x: calculate_parameter_deviation(x, 0, deviation_metric)
    )
    df_filtered[f"L0_b_{deviation_metric}"] = df_filtered["system_key"].apply(
        lambda x: calculate_parameter_deviation(x, 1, deviation_metric)
    )
    df_filtered[f"L1_b_{deviation_metric}"] = df_filtered["system_key"].apply(
        lambda x: calculate_parameter_deviation(x, 2, deviation_metric)
    )
    ...
    for param_col, param_name, param_title in l_params:
        ...
        plt.plot(
            [min_val, max_val], [min_val, max_val], "k--", label="y = x", alpha=0.5
        )
        ...
        print(
            f"{param_name} {metric_name} range: {df_filtered[param_col].min():.2f} - {df_filtered[param_col].max():.2f}"
        )

    # Print overall statistics
    print(f"\nSummary for {title_suffix} using {metric_name}:")
    print(
        f"Systems with complete binary L parameters: {len(df_filtered[df_filtered[f'L0_a_{deviation_metric}'] > 0])}"
    )
    ...
    df_filtered["L0_a_avg_mag"] = df_filtered["system_key"].apply(
        lambda x: calculate_parameter_avg_magnitude(x, 0)
    )
    df_filtered["L0_b_avg_mag"] = df_filtered["system_key"].apply(
        lambda x: calculate_parameter_avg_magnitude(x, 1)
    )
    df_filtered["L1_b_avg_mag"] = df_filtered["system_key"].apply(
        lambda x: calculate_parameter_avg_magnitude(x, 2)
    )
    ...
    for param_col, param_name, param_title in l_params_mag:
        ...
        # Print statistics for this parameter
        print(
            f"{param_name} Average Magnitude range: {df_filtered[param_col].min():.2f} - {df_filtered[param_col].max():.2f}"
        )

    print(f"\nAverage Magnitude Summary for {title_suffix}:")
    print(
        f"Systems with complete binary L parameters: {len(df_filtered[df_filtered['L0_a_avg_mag'] > 0])}"
    )


def plot_L_parameter_distributions(
    meta_data_path="all_dumps/gliq_manu_test3/ternary_Gliq_meta_final_linear.json",
):
    """
    ...
    for system_key, system_data in meta_data.items():
        ...
        for binary_key, params in binary_params.items():
            if len(params) >= 4:  # Ensure we have all 4 parameters
                ...
                L1_b_values.append(
                    params[2]
                )  # L1_b (note: you mentioned L1_a but the data shows L1_b)
    ...
    print(
        f"Collected {len(L0_a_values)} L parameter sets from {len(meta_data)} ternary systems"
    )
    ...
    print(
        f"L0_a: Mean = {mean_L0_a:.2f}, Std = {std_L0_a:.2f}, Min = {np.min(L0_a_values):.2f}, Max = {np.max(L0_a_values):.2f}"
    )
    print(
        f"L0_b: Mean = {mean_L0_b:.2f}, Std = {std_L0_b:.2f}, Min = {np.min(L0_b_values):.2f}, Max = {np.max(L0_b_values):.2f}"
    )
    print(
        f"L1_b: Mean = {mean_L1_b:.2f}, Std = {std_L1_b:.2f}, Min = {np.min(L1_b_values):.2f}, Max = {np.max(L1_b_values):.2f}"
    )
    ...
    for values, param_name, color in parameters:
        ...
        n, bins, patches = plt.hist(
            values, bins=50, alpha=0.7, color=color, edgecolor="black"
        )
        ...
        plt.axvline(
            q25, color="orange", linestyle=":", linewidth=1, label=f"Q25: {q25:.1f}"
        )
        plt.axvline(
            q75, color="orange", linestyle=":", linewidth=1, label=f"Q75: {q75:.1f}"
        )
