...
class ternary_interpolation:
    ...
    def ternary_interpolation(self): # maybe there's a better name for this than the same as the class name?
        ...
        x_B, x_C = self.tern_comp['B'], self.tern_comp['C']
        ...
        elif self.param_format in ['combined', 'comb-exp']:
            l_expr = _L_LIN_EXP_EXPR
        else:
            raise ValueError("param_format must be one of 'linear', 'combined', or 'comb-exp'")
    ...
    def get_phasedia_entries(self, sys):
        ...
        if os.path.exists(json_path):
            ...
            print(f"Caching ternary DFT entry data as {json_path}...")
